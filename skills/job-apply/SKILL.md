...
When invoked, check whether the user provided a specific job URL:

- **No URL given** → run the **Notion-Driven Workflow** below (pull approved jobs from Notion)
- **URL given** → skip to **Phase 1: Profile Setup** and treat that URL as the single job to apply to

---

## Notion-Driven Workflow

### Step 1: Query Notion for approved Direct Apply jobs

```bash
cd /Users/rukaiyakhan/Documents/Project/career-outreach-engine && \
  uv run python src/apply_helper.py get-jobs
```

This returns a JSON array of rows where `Approved = true` and `Path = Direct Apply`.

If the array is empty, say:
> No approved Direct Apply jobs found in Notion. Open your Notion database, check the **Approved** box on jobs you want to apply to, then run `/job-apply` again.

Then **stop**.

### Step 2: Show the job list

Display the jobs to the user:

```
Found N approved Direct Apply jobs:

1. Software Engineer Intern — Acme Corp  (Greenhouse)
   https://boards.greenhouse.io/acme/jobs/123

2. Frontend Engineer Intern — TechCo  (LinkedIn)
   https://linkedin.com/jobs/view/456
...
```

Ask: **Apply to all, or pick specific ones?**

### Step 3: For each selected job

Repeat the following for each job:

#### 3a. Select the best resume

```bash
cd /Users/rukaiyakhan/Documents/Project/career-outreach-engine && \
  uv run python src/apply_helper.py select-resume --page-id {page_id}
```

This reads the JD from Notion (or scrapes from the job URL if not stored), then picks the best-matching resume from the `resumes/` directory using an LLM.

Output: `{"resume": "filename.pdf", "reason": "...", "resume_path": "/absolute/path/to/file.pdf"}`

Tell the user:
> Applying to **{job_title}** at **{company}** using **{resume}** ({reason})

#### 3b. Apply using browser automation

Use the job URL and platform from the Notion row. Follow the platform-specific guidance in **Phase 2: Application Filling** below.

Use `resume_path` from the previous step for all file upload steps.

#### 3c. Update Notion after successful submission

```bash
cd /Users/rukaiyakhan/Documents/Project/career-outreach-engine && \
  uv run python src/apply_helper.py mark-applied --page-id {page_id} --resume {resume_filename}
```

This sets `Status = Applied`, unchecks `Approved`, records `Resume Used`, and sets `Last Contacted` to today.

Confirm to the user:
> ✓ Applied to **{job_title}** at **{company}** — Notion updated.

If the application fails (form error, login required, etc.), skip `mark-applied`, tell the user what happened, and move on to the next job.

---

## Initial Prompt

When this skill is invoked, first check if a profile exists at `/Users/rukaiyakhan/Documents/Project/career-outreach-engine/claude-job-profile.json`.
...
> Welcome back! Your profile is loaded from `/Users/rukaiyakhan/Documents/Project/career-outreach-engine/claude-job-profile.json`.
>
...
Your extracted profile is stored at `/Users/rukaiyakhan/Documents/Project/career-outreach-engine/claude-job-profile.json` for reuse across sessions. Run with `--reset-profile` to re-extract from resume.
...
If no profile exists at `/Users/rukaiyakhan/Documents/Project/career-outreach-engine/claude-job-profile.json`, or if the user requests a reset:
...
4. **Save confirmed profile** to `/Users/rukaiyakhan/Documents/Project/career-outreach-engine/claude-job-profile.json`

### Phase 2: Application Filling

1. **Load profile** from `/Users/rukaiyakhan/Documents/Project/career-outreach-engine/claude-job-profile.json`
